...
public class StatementPrinter {
    public static final int THISAMOUNT = 40000;
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            result.append(String.format("  %s: %s (%s seats)%n",
                    getPlay(p).getName(), used(getAmount(p)), p.getAudience()));
            totalAmount += getAmount(p);
        }
        result.append(String.format("Amount owed is %s%n", used(totalAmount)));
        result.append(String.format("You earned %s credits%n", volumeCredits));
        return result.toString();
    }

    private String used(int amount) {
        return NumberFormat.getCurrencyInstance(Locale.US).format(amount / CENTS_PER_DOLLAR);
    }
        ...
        switch (getPlay(perf).getType()) {
            case "tragedy":
                if (perf.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    result +=
                            TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON * (perf.getAudience() - TRAGEDY_AUDIENCE_THRESHOLD);
                }
                break;
            case "comedy":
