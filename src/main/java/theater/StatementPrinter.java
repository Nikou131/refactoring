...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            int thisAmount = THISAMOUNT;
            switch (play.type) {
                case "tragedy":
                    if (p.audience > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON * (p.audience - TRAGEDY_AUDIENCE_THRESHOLD);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n",
                    play.name, frmt.format(thisAmount / CENTS_PER_DOLLAR), p.audience));
